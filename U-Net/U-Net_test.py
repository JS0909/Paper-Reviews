...
root_path = 'F:\study_home\_data/team_project\cityscapes_data/'
...
#============ inference ===============
inferenceimg_path = 'F:\study_home\_data/team_project\predict_img/07.jpg'
inferenceimg_ = Image.open(inferenceimg_path)
inferenceimg_ = inferenceimg_.resize((256,256))
inferenceimg = np.array(inferenceimg_)
...
print(inferenceimg.shape)
# inferenceimg = inverse_transform(inferenceimg).permute(1, 2, 0).cpu().detach().numpy()
label_class_predicted = infer[0].cpu().detach().numpy()
# label_class_predicted = label_class_predicted.resize(np.array(inferenceimg_).shape[0], np.array(inferenceimg_).shape[1])
